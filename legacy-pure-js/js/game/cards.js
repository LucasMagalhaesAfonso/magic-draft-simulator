const CardEngine = {
  ...
  hasKeyword(card, keyword, gameState = null) {
    ...
    // Check dragon grants (Call the Spirit Dragons, etc.)
    if (gameState && this.hasCreatureType(card, 'Dragon')) {
      const cardController = this._findCardController(gameState, card);
      if (cardController !== null) {
        const battlefield = gameState.players[cardController].zones.battlefield.cards;
        for (const granter of battlefield) {
          if (granter._grantDragons) {
            // _grantDragons can be a single keyword or array of keywords
            const grantedKeywords = Array.isArray(granter._grantDragons) ? granter._grantDragons : [granter._grantDragons];
            if (grantedKeywords.some(kw => typeof kw === 'string' && kw.toLowerCase() === keyword.toLowerCase())) {
              return true;
            }
          }
        }
