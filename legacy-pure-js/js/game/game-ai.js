  ...
  playMainPhase(state, playerId) {
    ...
    while (playedSomething) {
      ...
      if (scored.length > 0) {
        ...
        // Calcular CMC reduzido via helper centralizado
        ...
        if (!ManaSystem.canAfford(state, playerId, card, null, reducedCmc)) {
          ...
          if (CardEngine.hasConvoke(card)) {
            ...
            if (convokeContrib.count > 0) {
              ...
              const requiredTotal = reducedCmc;
        ...
        if (result.success) {
          ...
          // If spell is pending on stack (waiting for human response), stop casting more
          ...
          if (effect.target === 'creature' || effect.target === 'opponent_creature') {
            ...
            const opCreatures = state.players[opponentId].zones.battlefield.cards
              ...
              .sort((a, b) => {
                ...
                const aScore = this._threatScore(a);
                const bScore = this._threatScore(b);
