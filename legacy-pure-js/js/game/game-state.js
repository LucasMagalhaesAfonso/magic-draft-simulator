const GameState = {
    ...
    for (const tempTrigger of tempTriggers) {
      ...
      if (shouldFireTemp) {
        ...
        for (const effect of tempTrigger.effects) {
          const result = this._resolveSimpleEffect(state, tempTrigger.controller, effect, { cardUid: tempTrigger.sourceCard?._uid });
          if (result) logs.push(result);
        }
  ...
  resolveUnlessPay(state, shouldPay) {
    ...
    if (shouldPay) {
      ...
      this.autoTapForSpell(state, opponentId, payStr, cost);
  ...
  castSpell(state, playerId, cardUid, targets, castingAdventure, castingEvoke) {
    ...
    if (!isAdventure && !state._resumingFromStackPriority && !state._pendingCastOnStack) {
      ...
      if (isOpponentHuman) {
        ...
        const hasAbilities = this.hasAffordableAbilities(state, opponentId);
        ...
        if (playableCardsCount > 0 || hasAbilities) {
          ...
          if (state._spellsThisTurn[playerId] === 2) {
            state.log.push(...this.fireTrigger(state, 'second_spell', { playerId, lastSpell: card, cardUid: card._uid }));
          }
          const castData = { playerId, cardUid: card._uid || '', card };
          state.log.push(...this.fireTrigger(state, 'cast_spell', castData));
          if (!CardEngine.isCreature(card)) state.log.push(...this.fireTrigger(state, 'cast_noncreature', castData));
          if (!CardEngine.isCreature(card) || CardEngine.hasCreatureType(card, 'Dragon')) state.log.push(...this.fireTrigger(state, 'cast_noncreature_or_dragon', castData));
          if ((state._spellsThisTurn[playerId] || 0) >= 2) state.log.push(...this.fireTrigger(state, 'cast_with_another_spell', castData));
    ...
    if (CardEngine.isPermanent(card)) {
      ...
      if (etbEffects.length > 0) {
        ...
        // If ETB set waitingForInput (search_library, scry, surveil, etc.), pause here
        // The game will resume after the human player completes their input
        if (state.waitingForInput && state.waitingForInput.type === 'search_library') {
      ...
      // VFX + log (new stack priority gate already handled human opponent case)
      if (typeof VFX !== 'undefined') VFX.spellCast(card.name);
      state.log.push(`${playerLabel} lanca ${card.name}.`);
   ...
   */
  getEffectiveCmcWithReduction(state, playerId, card) {
    const bf = state.players[playerId].zones.battlefield;
