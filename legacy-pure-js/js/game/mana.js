const ManaSystem = {
  ...
  // Convert shorthand mana cost "2BB" → "{2}{B}{B}", "1G" → "{1}{G}", "B" → "{B}"
  // If already in brace format like "{1}{G}", returns as-is
  formatManaCost(costStr) {
    if (!costStr) return '';
  ...
  canPay(pool, manaCost, cmc) {
    ...
    if (!cost.hybrids || cost.hybrids.length === 0) {
      // Cost reduction: if cmc < parsed total, reduce generic portion (same as payMana/autoTapForSpell)
      if (cmc !== undefined && cmc >= 0 && cost.total > cmc) {
        const reduction = cost.total - cmc;
        cost.generic = Math.max(0, cost.generic - reduction);
        cost.total = cmc;
      }
      // Extra safety: if parsed total < cmc (missing symbols?), use cmc as minimum
  ...
  _canPayCombination(pool, cost, hybridChoices) {
    ...
    for (const choice of hybridChoices) {
      if (/^\d+$/.test(choice)) {
        ...
        // Color choice (e.g., "W" from {2/W}) - initialize if not present
        if (tempColored[choice] === undefined) {
          tempColored[choice] = 0;
        }
        tempColored[choice]++;
      }
    }
  ...
  canAfford(state, playerId, card, targetCard = null, overrideCmc = undefined) {
    const bf = state.players[playerId].zones.battlefield;
    const availableLands = bf.cards.filter(c => CardEngine.isLand(c) && !c._tapped);
    const available = this.createPool();
    ...
    // Se overrideCmc foi passado (custo reduzido), usar ele em vez do original
    const requiredTotal = (overrideCmc !== undefined) ? overrideCmc : (effectiveCost.total || card.cmc || 0);

    if (actualTotal < requiredTotal) return false;

    return this.canPay(available, effectiveManaCost, requiredTotal);
  },
  ...
  _payWithHybrids(pool, cost) {
    ...
    // Find the best valid combination (one that leaves the most mana in pool)
    let bestPool = null;
    let bestRemaining = -1;
    ...
    // Return best combo, or original pool as fallback
