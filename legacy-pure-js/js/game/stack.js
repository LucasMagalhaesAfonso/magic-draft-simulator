      ...
      switch (effect.type) {
            ...
            for (const target of targets) {
              ...
              if (target.type === 'creature') {
                ...
                if (creature) {
                  ...
                  if (!this._payWardCost(creature, controller, gameState, log)) continue;
                ...
                if (typeof VFX !== 'undefined') VFX.playerDamage(target.player);
              }
            }
          ...
          if (targets && targets.length > 0) {
            ...
            targets.forEach((target, idx) => {
              ...
              if (target.type === 'creature') {
                const bf = gameState.players[target.player].zones.battlefield;
                const creature = bf.get(target.uid);
                if (creature && CardEngine.canBeTargeted(creature, controller)) {
                  if (typeof VFX !== 'undefined') VFX.damage(creature._uid);
        ...
        case 'sacrifice': {
          ...
          if (targets && targets.length > 0) {
            ...
            if (typeof VFX !== 'undefined') VFX.destroy(target.uid);
            ...
            let sacrificeable = bf.cards.filter(c => {
              ...
              if (effect.target === 'creature') return CardEngine.isCreature(c);
            ...
            if (effect.optional && controller !== 0) {
              ...
              const shouldSacrifice = sacrificeable.some(c =>
                ...
                CardEngine.getPower(c) <= 1 // Sacrifice weak creatures
              );
              if (!shouldSacrifice) {
            ...
            if (controller === 0) {
              ...
              sacrificeable.sort((a, b) => {
                ...
                if (CardEngine.isCreature(a) && CardEngine.isCreature(b)) {
        ...
        case 'damage_each_opponent': {
          ...
          for (const oppId of opponents) {
            ...
            if (typeof VFX !== 'undefined') VFX.playerDamage(oppId);
          }
        ...
        case 'optional_discard_draw': {
          ...
          if (controller === 0) {
            ...
            if (hand.length > 4) {
              ...
              const spells = hand.filter(c => !CardEngine.isLand(c));
              let worstCard = null;
              if (lands.length > 3) {
          ...
          if (gameState.players[controller].isHuman) {
            ...
            if (shouldDiscard) {
              ...
              if (preferNonland) {
                ...
                const nonlands = optHand.getAll().filter(c => !CardEngine.isLand(c));
                const lands = optHand.getAll().filter(c => CardEngine.isLand(c));
              ...
              // Track nonland discard for conditions
              const discardedNonland = !CardEngine.isLand(toDiscard);
              if (discardedNonland) {
                if (!gameState._lastDiscardedNonland) gameState._lastDiscardedNonland = {};
                gameState._lastDiscardedNonland[controller] = true;
              }
                ...
                for (const bonusEffect of effect.onNonlandDiscard) {
                  if (bonusEffect.type === 'damage' && bonusEffect.target === 'creature') {
                    ...
                    if (oppCreatures.length > 0) {
                      ...
                      if (bestTarget && CardEngine.canBeTargeted(bestTarget, controller)) {
                        bestTarget._damage = (bestTarget._damage || 0) + bonusEffect.amount;
                        if (typeof VFX !== 'undefined') VFX.damage(bestTarget._uid);
                        if (bestTarget._damage >= CardEngine.getToughness(bestTarget)) {
        ...
        case 'debuff_all': {
          if (effect.type === 'debuff_all') {
            ...
            if (effect.target === 'opponent_creatures') {
              ...
              allCreatures = gameState.players[targetId].zones.battlefield.cards.filter(c => CardEngine.isCreature(c));
            ...
            if (effect.target === 'non_dragon_creatures') {
              ...
              // Kill creatures with 0 or less toughness
              const dying = allCreatures.filter(c => CardEngine.getToughness(c) <= 0);
        ...
        case 'counter_spell': {
          ...
          if (effect.unless_pay !== undefined) {
            ...
            const wasDragonBeheld = !!(gameState._beholding && gameState._beholding[controller]);
            ...
            // Use the mana system to check affordability (lands + pool + mana abilities)
            const fakeCard = { mana_cost: costStr, cmc: finalCost };
            const canPay = ManaSystem.canAfford(gameState, opponentId, fakeCard);
            console.log(`[COUNTER_SPELL] Unless pay {${finalCost}}: canPay=${canPay}, beheld=${wasDragonBeheld}`);

            if (canPay) {
              if (gameState.players[opponentId].isHuman && opponentId === 0) {
                // Human player: ask for decision
                gameState._pendingUnlessPay = {
                  spell: targetSpell,
                  cost: finalCost,
                  costStr: costStr,
                ...
                gameState.waitingForInput = { type: 'unless_pay_decision', playerId: opponentId };
                return null; // Pause for human decision
              } else {
                // AI pays to save the spell - use autoTap + payMana (same system as casting)
                GameState.autoTapForSpell(gameState, opponentId, costStr, finalCost);
                gameState.manaPool[opponentId] = ManaSystem.payMana(gameState.manaPool[opponentId], costStr, finalCost);
                const label = wasDragonBeheld ? `{${finalCost}} (Dragon beheld)` : `{${finalCost}}`;
                log.push(`${targetSpell.name} nao foi anulado (${opponentId === 0 ? 'Voce' : 'IA'} pagou ${label}).`);
        ...
        case 'damage_all': {
          ...
          // Check if targeting creatures_and_planeswalkers (Dragonback Assault)
          if (effect.target === 'creatures_and_planeswalkers') {
            for (const pid of [0, 1]) {
              const bf = gameState.players[pid].zones.battlefield;
              const creatures = bf.cards.filter(c => CardEngine.isCreature(c));
              const planeswalkers = bf.cards.filter(c => CardEngine.isPlaneswalker(c));
              const dying = [];
              ...
              // Damage planeswalkers
              for (const pw of planeswalkers) {
                GameState.damagePlaneswalker(gameState, pw, dmgAllAmt, pid);
            ...
            // Regular damage_all (creatures only)
            for (const pid of [0, 1]) {
              const bf = gameState.players[pid].zones.battlefield;
              const creatures = bf.cards.filter(c => CardEngine.isCreature(c));
              const dying = [];
        ...
        case 'exile_from_graveyard': {
          ...
          if (efgCards.length > 0) {
            ...
            if (effect.choose_cards && controller === 0) {
              ...
              // For "exact_amount" effects with optional, must choose exactly X or 0 (all or nothing)
              let maxAmount, minAmount;
              if (effect.exact_amount && effect.optional) {
                // Exact amount or nothing (e.g., Aegis Sculptor: exactly 2 or 0)
                maxAmount = efgAmt;
                minAmount = 0; // Can decline (choose 0)
              ...
              if (pickedCount > 0) {
                gameState._exiledThisResolution = true;
              }
            }
          }
          break;
        ...
        case 'register_temp_trigger': {
          // Thunder of Unity: "Whenever a creature you control enters this turn, drain 1"
          // All-Out Assault: "When you next attack this turn, untap all creatures" (once: true)
          // Register a temporary trigger that expires at end of turn
          ...
          const tempTrigger = {
            ...
            controller: controller,
            sourceCard: card,
            expiresAt: effect.duration === 'this_turn' ? 'end_of_turn' : 'end_of_turn',
            once: effect.once || false // All-Out Assault: trigger once then remove
          };

          gameState._tempTriggers.push(tempTrigger);
          log.push(`Trigger temporario registrado: ${effect.event}${effect.once ? ' (uma vez)' : ''}.`);
          break;
        }
        ...
        case 'search_library': {
          ...
          if (effect.condition === 'control_dragon') {
            ...
            if (hasDragon) {
              ...
              if (effect.if_true) {
                ...
                toBattlefield = !toTop && !toHand; // BF if neither top nor hand
          ...
          console.log('[SEARCH_LIBRARY] Controller:', controller, 'isHuman:', gameState.players[controller].isHuman);
          if (gameState.players[controller].isHuman && slCandidates.length > 0) {
            ...
            console.log('[SEARCH_LIBRARY] Destination - toTop:', toTop, 'toHand:', toHand, 'toBattlefield:', toBattlefield, 'tapped:', tappedDest);
            ...
            if (toHand) {
              ...
              // Fallback: to hand (should not happen with current logic)
              gameState.players[controller].zones.hand.add(picked);
              slLib.shuffle();
              log.push(`Busca ${picked.name} da biblioteca para a mão.`);
            }
        ...
        case 'become_copy': {
          // This creature becomes a copy of target creature (permanent transformation)
          // Used by Naga Fleshcrafter's ETB
          const selfCreature = gameState.players[controller].zones.battlefield.get(card._uid);
          if (selfCreature && targets && targets.length > 0) {
            const copyTarget = targets[0];
            const templateCreature = gameState.players[copyTarget.player].zones.battlefield.get(copyTarget.uid);
            if (templateCreature && CardEngine.isCreature(templateCreature)) {
              // Preserve counters and equipment attachments
              const preservedCounters = selfCreature._counters ? { ...selfCreature._counters } : {};
              const preservedAttachments = selfCreature._attachments ? [...selfCreature._attachments] : [];
              ...
              selfCreature.toughness = templateCreature.toughness || CardEngine.getToughness(templateCreature);
              ...
              // Restore counters and attachments
        ...
        case 'mass_clone': {
          // All other creatures you control become copies of target creature until end of turn
          // Used by Naga Fleshcrafter's graveyard ability
          if (targets && targets.length > 0) {
            const templateTarget = targets[0];
            const templateCreature = gameState.players[templateTarget.player].zones.battlefield.get(templateTarget.uid);
            if (templateCreature && CardEngine.isCreature(templateCreature)) {
              const myCreatures = gameState.players[controller].zones.battlefield.cards.filter(
                c => CardEngine.isCreature(c) && c._uid !== templateCreature._uid
              );

              myCreatures.forEach(creature => {
                // Save original card data if not already saved
                ...
                // Copy stats and abilities from template
                ...
                creature.toughness = templateCreature.toughness || CardEngine.getToughness(templateCreature);
