...
describe("Bilbo, Retired Burglar", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["bilbo, retired burglar"]).toBeDefined();
  });
  ...
  it('has "leaves_battlefield" trigger in DB', () => {
    const db = CardEffectsDB['bilbo, retired burglar'];
    const trigger = db.triggered?.find((t: any) => t.event === "leaves_battlefield");
    expect(trigger).toBeDefined();
  });

  it('[trigger:leaves_battlefield] has ring_tempts effect in DB', () => {
    const db = CardEffectsDB['bilbo, retired burglar'];
    const json = JSON.stringify(db);
    expect(json).toContain("ring_tempts");
  });
  ...
  it('[trigger:combat_damage_player] creates 1 Treasure token(s)', () => {
    const game = new TestGame();
    game.resolveEffect(0, {"type":"create_token","power":0,"toughness":0,"name":"Treasure"});
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "bilbo, retired burglar", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
