...
describe("Borne Upon A Wind", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["borne upon a wind"]).toBeDefined();
  });

  it('[cast] has grant_flash_all_spells effect in DB', () => {
    const db = CardEffectsDB['borne upon a wind'];
    const json = JSON.stringify(db);
    expect(json).toContain("grant_flash_all_spells");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "borne upon a wind", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
