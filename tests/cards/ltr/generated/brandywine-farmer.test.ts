...
describe("Brandywine Farmer", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["brandywine farmer"]).toBeDefined();
  });
  ...
  it('[trigger:leaves_battlefield] creates 1 Food token(s)', () => {
    const game = new TestGame();
    game.resolveEffect(0, {"type":"create_token","power":0,"toughness":0,"name":"Food"});
    const tokens = game.battlefield(0).filter((c: any) => c.name === "Food");
    expect(tokens.length).toBe(1);
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "brandywine farmer", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
