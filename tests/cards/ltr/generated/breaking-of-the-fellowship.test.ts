...
describe("Breaking Of The Fellowship", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["breaking of the fellowship"]).toBeDefined();
  });

  it('[cast] has bite effect in DB', () => {
    const db = CardEffectsDB['breaking of the fellowship'];
    const json = JSON.stringify(db);
    expect(json).toContain("bite");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "breaking of the fellowship", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
