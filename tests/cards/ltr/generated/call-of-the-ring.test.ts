...
describe("Call Of The Ring", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["call of the ring"]).toBeDefined();
  });

  it('has "upkeep" trigger in DB', () => {
    const db = CardEffectsDB['call of the ring'];
    const trigger = db.triggered?.find((t: any) => t.event === "upkeep");
    expect(trigger).toBeDefined();
  });

  it('[trigger:upkeep] has ring_tempts effect in DB', () => {
    const db = CardEffectsDB['call of the ring'];
  ...
  it('[trigger:ring_tempts] self loses 2 life', () => {
    const game = new TestGame();
    game.setLife(0, 20);
    game.resolveEffect(0, {"type":"loseLife","amount":2});
    expect(game.life(0)).toBe(20 - 2);
  });

  it('[trigger:ring_tempts] draws 1 card(s)', () => {
    const game = new TestGame();
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "call of the ring", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
