...
describe("Dawn Of A New Age", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["dawn of a new age"]).toBeDefined();
  });
  ...
  it('[trigger:end_step] has remove_counter_draw effect in DB', () => {
    const db = CardEffectsDB['dawn of a new age'];
    const json = JSON.stringify(db);
    expect(json).toContain("remove_counter_draw");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "dawn of a new age", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
