...
describe("Deceive The Messenger", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["deceive the messenger"]).toBeDefined();
  });

  it('[cast] has debuff effect in DB', () => {
    const db = CardEffectsDB['deceive the messenger'];
    const json = JSON.stringify(db);
    expect(json).toContain("debuff");
  });

  it('[cast] amasses 1 (creates/grows Orc Army)', () => {
    const game = new TestGame();
    game.resolveEffect(0, {"type":"amass","amount":1,"subtype":"Orcs"});
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "deceive the messenger", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
