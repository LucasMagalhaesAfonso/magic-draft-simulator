...
describe("Delighted Halfling", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["delighted halfling"]).toBeDefined();
  });
  ...
  it('[activated[1]] adds mana (any)', () => {
    const game = new TestGame();
    const before = game.state.manaPool[0].C || 0;
    game.resolveEffect(0, {"type":"add_mana","color":"C","uncounterable":true});
    expect(game.state.manaPool[0].C).toBeGreaterThan(before);
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "delighted halfling", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
