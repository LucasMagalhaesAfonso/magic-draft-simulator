...
describe("Dreadful As The Storm", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["dreadful as the storm"]).toBeDefined();
  });

  it('[cast] has set_base_pt effect in DB', () => {
    const db = CardEffectsDB['dreadful as the storm'];
    const json = JSON.stringify(db);
    expect(json).toContain("set_base_pt");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "dreadful as the storm", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
