...
describe("Dunland Crebain", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["dunland crebain"]).toBeDefined();
  });

  it('has keyword: flying', () => {
    const game = new TestGame();
    const card = game.addToBattlefield(0, { name: "dunland crebain", type_line: 'Creature', power: '1', toughness: '1', keywords: ["flying"] });
    expect(CardUtils.hasKeyword(card, "flying")).toBe(true);
  });

  it('[etb] amasses 2 (creates/grows Orc Army)', () => {
    const game = new TestGame();
    game.resolveEffect(0, {"type":"amass","amount":2,"subtype":"Orcs"});
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "dunland crebain", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
