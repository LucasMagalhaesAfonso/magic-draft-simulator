...
describe("Eagles Of The North", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["eagles of the north"]).toBeDefined();
  });

  it('has keyword: flying', () => {
    const game = new TestGame();
    const card = game.addToBattlefield(0, { name: "eagles of the north", type_line: 'Creature', power: '1', toughness: '1', keywords: ["flying"] });
    expect(CardUtils.hasKeyword(card, "flying")).toBe(true);
  });

  it('[etb] has buff_all effect in DB', () => {
    const db = CardEffectsDB['eagles of the north'];
    const json = JSON.stringify(db);
    expect(json).toContain("buff_all");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "eagles of the north", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
