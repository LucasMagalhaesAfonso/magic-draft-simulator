...
describe("Elven Farsight", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["elven farsight"]).toBeDefined();
  });

  it('[cast] scry 3 — sets _pendingScry for human', () => {
    const game = new TestGame();
    game.setHuman(0, true);
    for (let i = 0; i < 4; i++) game.addToLibraryTop(0, { name: 'Top', type_line: 'Creature' });
    game.resolveEffect(0, {"type":"scry","amount":3});
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "elven farsight", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
