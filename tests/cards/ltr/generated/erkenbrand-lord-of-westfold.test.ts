...
describe("Erkenbrand, Lord Of Westfold", () => {
  ...
  it('has "enters_battlefield" trigger in DB', () => {
    const db = CardEffectsDB['erkenbrand, lord of westfold'];
    const trigger = db.triggered?.find((t: any) => t.event === "enters_battlefield");
    expect(trigger).toBeDefined();
  });

  it('[trigger:enters_battlefield] has buff_all effect in DB', () => {
    const db = CardEffectsDB['erkenbrand, lord of westfold'];
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "erkenbrand, lord of westfold", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
