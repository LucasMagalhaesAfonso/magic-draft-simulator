...
describe("Escape From Orthanc", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["escape from orthanc"]).toBeDefined();
  });

  it('[cast] buffs creature +1/+3 (end_of_turn)', () => {
    const game = new TestGame();
    const creature = game.addToBattlefield(0, { name: 'Target', type_line: 'Creature', power: '1', toughness: '1' });
    game.resolveEffect(0, {"type":"buff","power":1,"toughness":3,"target":"own_creature","duration":"end_of_turn","keywords":["flying"]});
  ...
  it('[cast] has untap effect in DB', () => {
    const db = CardEffectsDB['escape from orthanc'];
    const json = JSON.stringify(db);
    expect(json).toContain("untap");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "escape from orthanc", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
