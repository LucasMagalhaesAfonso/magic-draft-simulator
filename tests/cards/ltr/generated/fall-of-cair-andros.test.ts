...
describe("Fall Of Cair Andros", () => {
  ...
  it('has "excess_noncombat_damage" trigger in DB', () => {
    const db = CardEffectsDB['fall of cair andros'];
    const trigger = db.triggered?.find((t: any) => t.event === "excess_noncombat_damage");
    expect(trigger).toBeDefined();
  });

  it('[activated[0]] deals 7 damage to creature', () => {
    const game = new TestGame();
    const target = game.addToBattlefield(1, { name: 'Target', type_line: 'Creature — Beast', power: '1', toughness: '10' });
    game.resolveEffect(0, {"type":"damage","amount":7,"target":"creature"}, { targetUid: target._uid });
    expect(target._damage || 0).toBe(7);
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "fall of cair andros", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
