...
describe("Faramir, Field Commander", () => {
  ...
  it('has "end_step" trigger in DB', () => {
    const db = CardEffectsDB['faramir, field commander'];
    const trigger = db.triggered?.find((t: any) => t.event === "end_step");
    expect(trigger).toBeDefined();
  });

  it('[trigger:end_step] draws 1 card(s)', () => {
  ...
  it('[trigger:ring_tempts] creates 1 Human Soldier token(s)', () => {
    const game = new TestGame();
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "faramir, field commander", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
