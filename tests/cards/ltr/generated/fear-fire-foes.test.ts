...
describe("Fear, Fire, Foes!", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["fear, fire, foes!"]).toBeDefined();
  });

  it('[cast] has cant_prevent_damage effect in DB', () => {
    const db = CardEffectsDB['fear, fire, foes!'];
    const json = JSON.stringify(db);
    expect(json).toContain("cant_prevent_damage");
  });

  it('[cast] has damage_same_controller effect in DB', () => {
    const db = CardEffectsDB['fear, fire, foes!'];
    const json = JSON.stringify(db);
    expect(json).toContain("damage_same_controller");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "fear, fire, foes!", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
