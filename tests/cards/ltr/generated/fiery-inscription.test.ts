...
describe("Fiery Inscription", () => {
  ...
  it('[etb] has ring_tempts effect in DB', () => {
    const db = CardEffectsDB['fiery inscription'];
  ...
  it('[trigger:cast_noncreature] deals 2 damage to player', () => {
    const game = new TestGame();
    game.setLife(1, 20);
    game.resolveEffect(0, {"type":"damage","amount":2,"target":"opponent"});
    expect(game.life(1)).toBe(20 - 2);
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "fiery inscription", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
