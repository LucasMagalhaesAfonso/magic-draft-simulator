...
describe("Frodo Baggins", () => {
  ...
  it('[etb] has ring_tempts effect in DB', () => {
    const db = CardEffectsDB['frodo baggins'];
  ...
  it('[trigger:enters_battlefield] has ring_tempts effect in DB', () => {
    const db = CardEffectsDB['frodo baggins'];
    const json = JSON.stringify(db);
    expect(json).toContain("ring_tempts");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "frodo baggins", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
