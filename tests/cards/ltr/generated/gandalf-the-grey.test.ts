...
describe("Gandalf The Grey", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["gandalf the grey"]).toBeDefined();
  });
  ...
  it('[trigger:cast_instant_sorcery] modal has 4 mode(s)', () => {
    const db = CardEffectsDB['gandalf the grey'];
    const json = JSON.stringify(db);
    expect(json).toContain('modal');
  });

  it('[trigger:cast_instant_sorcery] modal sets _pendingModal for human player', () => {
    const game = new TestGame();
    game.setHuman(0, true);
    game.resolveEffect(0, {"type":"modal","unique_modes":true,"modes":[[{"type":"tap_or_untap","target":"permanent"}],[{"type":"damage","amount":3,"target":"each_opponent"}],[{"type":"copy_spell","target":"own_instant_sorcery"}],[{"type":"put_on_top_library","target":"self"}]]}, {});
    expect(game.state._pendingModal).toBeDefined();
    expect(game.state._pendingModal?.modes.length).toBe(4);
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "gandalf the grey", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
