...
describe("Gimli, Mournful Avenger", () => {
  ...
  it('has "other_creature_dies" trigger in DB', () => {
    const db = CardEffectsDB['gimli, mournful avenger'];
    const trigger = db.triggered?.find((t: any) => t.event === "other_creature_dies");
    expect(trigger).toBeDefined();
  });

  it('[trigger:other_creature_dies] has counter_self effect in DB', () => {
    const db = CardEffectsDB['gimli, mournful avenger'];
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "gimli, mournful avenger", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
