...
describe("Glamdring", () => {
  ...
  it('has "combat_damage_player" trigger in DB', () => {
    const db = CardEffectsDB['glamdring'];
    const trigger = db.triggered?.find((t: any) => t.event === "combat_damage_player");
    expect(trigger).toBeDefined();
  });

  it('[trigger:combat_damage_player] draws 1 card(s)', () => {
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "glamdring", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
