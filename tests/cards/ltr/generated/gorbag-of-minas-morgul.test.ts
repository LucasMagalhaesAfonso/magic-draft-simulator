...
describe("Gorbag Of Minas Morgul", () => {
  ...
  it('has "combat_damage_player" trigger in DB', () => {
    const db = CardEffectsDB['gorbag of minas morgul'];
    const trigger = db.triggered?.find((t: any) => t.event === "combat_damage_player");
    expect(trigger).toBeDefined();
  });

  it('[trigger:combat_damage_player] has sacrifice effect in DB', () => {
    const db = CardEffectsDB['gorbag of minas morgul'];
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "gorbag of minas morgul", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
