...
describe("Great Hall Of The Citadel", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["great hall of the citadel"]).toBeDefined();
  });
  ...
  it('[activated[1]] adds mana (any)', () => {
    const game = new TestGame();
    const before = game.state.manaPool[0].C || 0;
    game.resolveEffect(0, {"type":"add_mana","color":"C","amount":2});
    expect(game.state.manaPool[0].C).toBeGreaterThan(before);
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "great hall of the citadel", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
