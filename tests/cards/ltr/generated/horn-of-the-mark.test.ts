...
describe("Horn Of The Mark", () => {
  ...
  it('has "attacks" trigger in DB', () => {
    const db = CardEffectsDB['horn of the mark'];
    const trigger = db.triggered?.find((t: any) => t.event === "attacks");
    expect(trigger).toBeDefined();
  });

  it('[trigger:attacks] has look_top effect in DB', () => {
    const db = CardEffectsDB['horn of the mark'];
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "horn of the mark", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
