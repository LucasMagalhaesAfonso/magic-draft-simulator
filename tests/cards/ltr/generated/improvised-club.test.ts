...
describe("Improvised Club", () => {
  ...
  it('[cast] deals 4 damage to player', () => {
    const game = new TestGame();
    game.setLife(1, 20);
    game.resolveEffect(0, {"type":"damage","amount":4,"target":"any"});
    expect(game.life(1)).toBe(20 - 4);
  });

  it('has sacrifice additional cost (alternate_cost: none)', () => {
    const db = CardEffectsDB["improvised club"];
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "improvised club", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
