...
describe("Ioreth Of The Healing House", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["ioreth of the healing house"]).toBeDefined();
  });

  it('[activated[0]] has untap effect in DB', () => {
    const db = CardEffectsDB['ioreth of the healing house'];
    const json = JSON.stringify(db);
    expect(json).toContain("untap");
  });

  it('[activated[1]] has untap effect in DB', () => {
    const db = CardEffectsDB['ioreth of the healing house'];
    const json = JSON.stringify(db);
    expect(json).toContain("untap");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "ioreth of the healing house", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
