...
describe("Isolation At Orthanc", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["isolation at orthanc"]).toBeDefined();
  });

  it('[cast] has bounce_to_library_top effect in DB', () => {
    const db = CardEffectsDB['isolation at orthanc'];
    const json = JSON.stringify(db);
    expect(json).toContain("bounce_to_library_top");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "isolation at orthanc", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
