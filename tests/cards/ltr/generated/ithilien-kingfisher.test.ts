...
describe("Ithilien Kingfisher", () => {
  ...
  it('has keyword: flying', () => {
    ...
    const card = game.addToBattlefield(0, { name: "ithilien kingfisher", type_line: 'Creature', power: '1', toughness: '1', keywords: ["flying"] });
    expect(CardUtils.hasKeyword(card, "flying")).toBe(true);
  });

  it('has "dies" trigger in DB', () => {
    const db = CardEffectsDB['ithilien kingfisher'];
    const trigger = db.triggered?.find((t: any) => t.event === "dies");
    expect(trigger).toBeDefined();
  });

  it('[trigger:dies] draws 1 card(s)', () => {
    const game = new TestGame();
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "ithilien kingfisher", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
