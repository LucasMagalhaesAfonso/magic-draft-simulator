...
describe("Last March Of The Ents", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["last march of the ents"]).toBeDefined();
  });

  it('[cast] has put_creatures_from_hand effect in DB', () => {
    const db = CardEffectsDB['last march of the ents'];
    const json = JSON.stringify(db);
    expect(json).toContain("put_creatures_from_hand");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "last march of the ents", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
