...
describe("March From The Black Gate", () => {
  ...
  it('has "attacks" trigger in DB', () => {
    const db = CardEffectsDB['march from the black gate'];
    const trigger = db.triggered?.find((t: any) => t.event === "attacks");
    expect(trigger).toBeDefined();
  });

  it('[trigger:attacks] amasses 1 (creates/grows Orc Army)', () => {
    const game = new TestGame();
    game.resolveEffect(0, {"type":"amass","amount":1});
    const armies = game.battlefield(0).filter((c: any) => (c.type_line||'').toLowerCase().includes('army'));
    expect(armies.length).toBe(1);
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "march from the black gate", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
