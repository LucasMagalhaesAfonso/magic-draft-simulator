...
describe("Meriadoc Brandybuck", () => {
  ...
  it('has "attacks" trigger in DB', () => {
    const db = CardEffectsDB['meriadoc brandybuck'];
    const trigger = db.triggered?.find((t: any) => t.event === "attacks");
    expect(trigger).toBeDefined();
  });

  it('[trigger:attacks] creates 1 Food token(s)', () => {
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "meriadoc brandybuck", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
