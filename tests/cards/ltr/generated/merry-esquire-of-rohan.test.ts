...
describe("Merry, Esquire Of Rohan", () => {
  ...
  it('has keyword: haste', () => {
    const game = new TestGame();
    const card = game.addToBattlefield(0, { name: "merry, esquire of rohan", type_line: 'Creature', power: '1', toughness: '1', keywords: ["haste"] });
    expect(CardUtils.hasKeyword(card, "haste")).toBe(true);
  });

  it('has "attacks" trigger in DB', () => {
    const db = CardEffectsDB['merry, esquire of rohan'];
    const trigger = db.triggered?.find((t: any) => t.event === "attacks");
    expect(trigger).toBeDefined();
  });

  it('[trigger:attacks] draws 1 card(s)', () => {
    const game = new TestGame();
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "merry, esquire of rohan", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
