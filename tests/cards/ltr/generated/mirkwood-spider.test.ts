...
describe("Mirkwood Spider", () => {
  ...
  it('has keyword: deathtouch', () => {
    const game = new TestGame();
    const card = game.addToBattlefield(0, { name: "mirkwood spider", type_line: 'Creature', power: '1', toughness: '1', keywords: ["deathtouch"] });
    expect(CardUtils.hasKeyword(card, "deathtouch")).toBe(true);
  });

  it('has "attacks" trigger in DB', () => {
    const db = CardEffectsDB['mirkwood spider'];
    const trigger = db.triggered?.find((t: any) => t.event === "attacks");
    expect(trigger).toBeDefined();
  });

  it('[trigger:attacks] grants deathtouch to creature', () => {
    const game = new TestGame();
    ...
    game.resolveEffect(0, {"type":"grant","keywords":["deathtouch"],"target":"legendary_creature","duration":"end_of_turn"}, { targetUid: creature._uid });
    ...
    const hasTempKw = (creature._tempKeywords || []).some((tk: any) => {
      ...
      return k?.toLowerCase() === "deathtouch";
    });
    // Also accept the keyword baked in (static grant)
    const hasBaseKw = (creature.keywords || []).some((k: string) => k.toLowerCase() === "deathtouch");
    // If neither, the DB should at least contain the keyword definition
    const dbHasIt = JSON.stringify(CardEffectsDB['mirkwood spider']).toLowerCase().includes("deathtouch");
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "mirkwood spider", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
