...
describe("Mordor Trebuchet", () => {
  ...
  it('has keyword: defender', () => {
    const game = new TestGame();
    const card = game.addToBattlefield(0, { name: "mordor trebuchet", type_line: 'Creature', power: '1', toughness: '1', keywords: ["defender"] });
    expect(CardUtils.hasKeyword(card, "defender")).toBe(true);
  });

  it('has "attacks" trigger in DB', () => {
    const db = CardEffectsDB['mordor trebuchet'];
    const trigger = db.triggered?.find((t: any) => t.event === "attacks");
    expect(trigger).toBeDefined();
  });

  it('[trigger:attacks] creates 1 Ballistic Boulder token(s)', () => {
    const game = new TestGame();
    game.resolveEffect(0, {"type":"create_token","power":2,"toughness":1,"name":"Ballistic Boulder","type_line":"Artifact Creature — Construct","keywords":["flying"],"tapped":true,"attacking":true,"sacrifice_eoc":true});
    const tokens = game.battlefield(0).filter((c: any) => c.name === "Ballistic Boulder");
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "mordor trebuchet", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
