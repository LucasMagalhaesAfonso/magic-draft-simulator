...
describe("Moria Marauder", () => {
  ...
  it('has keyword: double strike', () => {
    const game = new TestGame();
    const card = game.addToBattlefield(0, { name: "moria marauder", type_line: 'Creature', power: '1', toughness: '1', keywords: ["double strike"] });
    expect(CardUtils.hasKeyword(card, "double strike")).toBe(true);
  });

  it('has "combat_damage_player" trigger in DB', () => {
    const db = CardEffectsDB['moria marauder'];
    const trigger = db.triggered?.find((t: any) => t.event === "combat_damage_player");
    expect(trigger).toBeDefined();
  });

  it('[trigger:combat_damage_player] has exile_top_play effect in DB', () => {
    const db = CardEffectsDB['moria marauder'];
    const json = JSON.stringify(db);
    expect(json).toContain("exile_top_play");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "moria marauder", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
