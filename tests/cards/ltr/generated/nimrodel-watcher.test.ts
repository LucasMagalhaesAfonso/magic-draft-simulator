...
describe("Nimrodel Watcher", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["nimrodel watcher"]).toBeDefined();
  });
  ...
  it('[trigger:scry] buffs self +1/+0 (end_of_turn)', () => {
    const game = new TestGame();
    const creature = game.addToBattlefield(0, { name: 'Target', type_line: 'Creature', power: '1', toughness: '1' });
    game.resolveEffect(0, {"type":"buff","power":1,"toughness":0,"target":"self","duration":"end_of_turn"}, { cardUid: creature._uid });
    expect(CardUtils.getPower(creature)).toBeGreaterThanOrEqual(1 + 1);
    
  });

  it('[trigger:scry] grants unblockable to creature', () => {
    const game = new TestGame();
    ...
    game.resolveEffect(0, {"type":"grant","keywords":["unblockable"],"target":"self","duration":"end_of_turn"}, { targetUid: creature._uid });
    ...
    const hasTempKw = (creature._tempKeywords || []).some((tk: any) => {
      ...
      return k?.toLowerCase() === "unblockable";
    });
    // Also accept the keyword baked in (static grant)
    const hasBaseKw = (creature.keywords || []).some((k: string) => k.toLowerCase() === "unblockable");
    // If neither, the DB should at least contain the keyword definition
    const dbHasIt = JSON.stringify(CardEffectsDB['nimrodel watcher']).toLowerCase().includes("unblockable");
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "nimrodel watcher", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
