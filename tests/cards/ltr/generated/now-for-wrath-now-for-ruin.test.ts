...
describe("Now For Wrath, Now For Ruin!", () => {
  ...
  it('[cast] has counter_all effect in DB', () => {
    const db = CardEffectsDB['now for wrath, now for ruin!'];
    const json = JSON.stringify(db);
    expect(json).toContain("counter_all");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "now for wrath, now for ruin!", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
