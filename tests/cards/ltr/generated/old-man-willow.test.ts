...
describe("Old Man Willow", () => {
  ...
  it('has "attacks" trigger in DB', () => {
    const db = CardEffectsDB['old man willow'];
    const trigger = db.triggered?.find((t: any) => t.event === "attacks");
    expect(trigger).toBeDefined();
  });

  it('[trigger:attacks] has debuff effect in DB', () => {
    const db = CardEffectsDB['old man willow'];
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "old man willow", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
