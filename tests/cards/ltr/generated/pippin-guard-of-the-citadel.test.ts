...
describe("Pippin, Guard Of The Citadel", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["pippin, guard of the citadel"]).toBeDefined();
  });

  it('has keyword: vigilance', () => {
    const game = new TestGame();
    const card = game.addToBattlefield(0, { name: "pippin, guard of the citadel", type_line: 'Creature', power: '1', toughness: '1', keywords: ["vigilance"] });
    expect(CardUtils.hasKeyword(card, "vigilance")).toBe(true);
  });

  it('[activated[0]] grants protection to creature', () => {
    const game = new TestGame();
    ...
    game.resolveEffect(0, {"type":"grant","keywords":["protection"],"target":"own_creature","duration":"end_of_turn"}, { targetUid: creature._uid });
    ...
    const hasTempKw = (creature._tempKeywords || []).some((tk: any) => {
      ...
      return k?.toLowerCase() === "protection";
    });
    // Also accept the keyword baked in (static grant)
    const hasBaseKw = (creature.keywords || []).some((k: string) => k.toLowerCase() === "protection");
    // If neither, the DB should at least contain the keyword definition
    const dbHasIt = JSON.stringify(CardEffectsDB['pippin, guard of the citadel']).toLowerCase().includes("protection");
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "pippin, guard of the citadel", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
