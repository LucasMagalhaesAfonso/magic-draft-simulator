...
describe("Radagast The Brown", () => {
  ...
  it('has "enters_battlefield" trigger in DB', () => {
    const db = CardEffectsDB['radagast the brown'];
    const trigger = db.triggered?.find((t: any) => t.event === "enters_battlefield");
    expect(trigger).toBeDefined();
  });

  it('[trigger:enters_battlefield] has look_top effect in DB', () => {
    const db = CardEffectsDB['radagast the brown'];
    const json = JSON.stringify(db);
    expect(json).toContain("look_top");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "radagast the brown", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
