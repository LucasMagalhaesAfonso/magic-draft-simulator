...
describe("Rally At The Hornburg", () => {
  ...
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB["rally at the hornburg"]).toBeDefined();
  });

  it('[cast] creates 2 Human Soldier token(s)', () => {
    const game = new TestGame();
    game.resolveEffect(0, {"type":"create_token","power":1,"toughness":1,"name":"Human Soldier","type_line":"Creature — Human Soldier","colors":["W"],"count":2});
    const tokens = game.battlefield(0).filter((c: any) => c.name === "Human Soldier");
    expect(tokens.length).toBe(2);
  });

  it('[cast] has grant_all effect in DB', () => {
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "rally at the hornburg", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
