...
describe("Rangers Of Ithilien", () => {
  ...
  it('has keyword: vigilance', () => {
    ...
    const card = game.addToBattlefield(0, { name: "rangers of ithilien", type_line: 'Creature', power: '1', toughness: '1', keywords: ["vigilance"] });
    expect(CardUtils.hasKeyword(card, "vigilance")).toBe(true);
  });

  it('[etb] has gain_control effect in DB', () => {
    const db = CardEffectsDB['rangers of ithilien'];
    const json = JSON.stringify(db);
    expect(json).toContain("gain_control");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "rangers of ithilien", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
