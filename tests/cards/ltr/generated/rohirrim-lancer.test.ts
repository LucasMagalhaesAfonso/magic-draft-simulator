...
describe("Rohirrim Lancer", () => {
  ...
  it('has keyword: menace', () => {
    ...
    const card = game.addToBattlefield(0, { name: "rohirrim lancer", type_line: 'Creature', power: '1', toughness: '1', keywords: ["menace"] });
    expect(CardUtils.hasKeyword(card, "menace")).toBe(true);
  });

  it('has "dies" trigger in DB', () => {
    const db = CardEffectsDB['rohirrim lancer'];
    const trigger = db.triggered?.find((t: any) => t.event === "dies");
    expect(trigger).toBeDefined();
  });

  it('[trigger:dies] has ring_tempts effect in DB', () => {
    const db = CardEffectsDB['rohirrim lancer'];
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "rohirrim lancer", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
