...
describe("Samwise Gamgee", () => {
  ...
  it('has "other_creature_enters" trigger in DB', () => {
    const db = CardEffectsDB['samwise gamgee'];
    const trigger = db.triggered?.find((t: any) => t.event === "other_creature_enters");
    expect(trigger).toBeDefined();
  });

  it('[trigger:other_creature_enters] creates 1 Food token(s)', () => {
  ...
  it('[activated[0]] has return_from_graveyard effect in DB', () => {
    const db = CardEffectsDB['samwise gamgee'];
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "samwise gamgee", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
