...
describe("Samwise The Stouthearted", () => {
  ...
  it('has keyword: flash', () => {
    ...
    const card = game.addToBattlefield(0, { name: "samwise the stouthearted", type_line: 'Creature', power: '1', toughness: '1', keywords: ["flash"] });
    expect(CardUtils.hasKeyword(card, "flash")).toBe(true);
  });

  it('[etb] has return_from_graveyard effect in DB', () => {
    const db = CardEffectsDB['samwise the stouthearted'];
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "samwise the stouthearted", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
