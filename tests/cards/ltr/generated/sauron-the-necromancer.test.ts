...
describe("Sauron, The Necromancer", () => {
  ...
  it('has keyword: menace', () => {
    const game = new TestGame();
    const card = game.addToBattlefield(0, { name: "sauron, the necromancer", type_line: 'Creature', power: '1', toughness: '1', keywords: ["menace"] });
    expect(CardUtils.hasKeyword(card, "menace")).toBe(true);
  });

  it('has "attacks" trigger in DB', () => {
    const db = CardEffectsDB['sauron, the necromancer'];
    const trigger = db.triggered?.find((t: any) => t.event === "attacks");
    expect(trigger).toBeDefined();
  });

  it('[trigger:attacks] creates 1 Zombie token(s)', () => {
    const game = new TestGame();
    game.resolveEffect(0, {"type":"create_token","power":3,"toughness":3,"name":"Zombie","type_line":"Creature — Zombie","keywords":["menace"],"tapped":true,"attacking":true});
    const tokens = game.battlefield(0).filter((c: any) => c.name === "Zombie");
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "sauron, the necromancer", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
