...
describe("Shelob, Child Of Ungoliant", () => {
  ...
  it('has keyword: deathtouch', () => {
    ...
    const card = game.addToBattlefield(0, { name: "shelob, child of ungoliant", type_line: 'Creature', power: '1', toughness: '1', keywords: ["deathtouch"] });
    expect(CardUtils.hasKeyword(card, "deathtouch")).toBe(true);
  });

  it('has "opponent_creature_dies_from_damage" trigger in DB', () => {
    const db = CardEffectsDB['shelob, child of ungoliant'];
    const trigger = db.triggered?.find((t: any) => t.event === "opponent_creature_dies_from_damage");
    expect(trigger).toBeDefined();
  });

  it('[trigger:opponent_creature_dies_from_damage] creates 1 Food token(s)', () => {
    const game = new TestGame();
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "shelob, child of ungoliant", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
