...
describe("Snarling Warg", () => {
  ...
  it('has keyword: menace', () => {
    ...
    const card = game.addToBattlefield(0, { name: "snarling warg", type_line: 'Creature', power: '1', toughness: '1', keywords: ["menace"] });
    expect(CardUtils.hasKeyword(card, "menace")).toBe(true);
  });

  it('has conditional_buff static (condition: control_goblin_or_orc)', () => {
    const db = CardEffectsDB['snarling warg'];
    const buff = db.static?.find((s: any) => s.type === 'conditional_buff' && s.condition === "control_goblin_or_orc");
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "snarling warg", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
