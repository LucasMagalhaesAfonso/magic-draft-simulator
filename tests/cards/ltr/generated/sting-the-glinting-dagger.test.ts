...
describe("Sting, The Glinting Dagger", () => {
  ...
  it('has "combat_begin" trigger in DB', () => {
    const db = CardEffectsDB['sting, the glinting dagger'];
    const trigger = db.triggered?.find((t: any) => t.event === "combat_begin");
    expect(trigger).toBeDefined();
  });

  it('[trigger:combat_begin] has untap effect in DB', () => {
    const db = CardEffectsDB['sting, the glinting dagger'];
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "sting, the glinting dagger", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
