...
describe("The Mouth Of Sauron", () => {
  ...
  it('[etb] amasses 3 (creates/grows Orc Army)', () => {
    const game = new TestGame();
    game.resolveEffect(0, {"type":"amass","amount":3});
    const armies = game.battlefield(0).filter((c: any) => (c.type_line||'').toLowerCase().includes('army'));
    expect(armies.length).toBe(1);
    expect(armies[0]._counters?.['+1/+1'] || 0).toBe(3);
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "the mouth of sauron", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
