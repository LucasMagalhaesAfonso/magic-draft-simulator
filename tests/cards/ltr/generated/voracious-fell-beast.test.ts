...
describe("Voracious Fell Beast", () => {
  ...
  it('has keyword: flying', () => {
    ...
    const card = game.addToBattlefield(0, { name: "voracious fell beast", type_line: 'Creature', power: '1', toughness: '1', keywords: ["flying"] });
    expect(CardUtils.hasKeyword(card, "flying")).toBe(true);
  });

  it('[etb] has sacrifice effect in DB', () => {
    const db = CardEffectsDB['voracious fell beast'];
    const json = JSON.stringify(db);
    expect(json).toContain("sacrifice");
  });
  ...
  describe('Cast flow', () => {
    it('AI can cast without crash', () => {
      ...
      const card = game.addToHand(0, { name: "voracious fell beast", type_line: 'Instant', mana_cost: '{1}', cmc: 1 });
