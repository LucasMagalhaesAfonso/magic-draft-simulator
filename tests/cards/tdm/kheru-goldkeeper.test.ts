...
describe('Kheru Goldkeeper', () => {
  it('exists in CardEffectsDB', () => {
    expect(CardEffectsDB['kheru goldkeeper']).toBeDefined();
  });

  it('has Flying', () => {
    const game = new TestGame();
    const card = game.addToBattlefield(0, { name: 'Kheru Goldkeeper', type_line: 'Creature — Dragon', power: '3', toughness: '3', keywords: ["Flying","Renew","Treasure"] });
    expect(CardUtils.hasKeyword(card, 'Flying')).toBe(true);
  });

  it('creates Treasure token', () => {
    // Verify CardEffectsDB contains expected mechanic
    const dbEntry = CardEffectsDB['kheru goldkeeper'];
